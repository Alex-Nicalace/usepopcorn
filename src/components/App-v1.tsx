...
type MovieData = (typeof tempMovieData)[0];
...
function NumResults({ movies }: { movies: MovieData[] }): JSX.Element {
  return (
...
function MovieList({ movies }: { movies: MovieData[] }): JSX.Element {
  return (
    <ul className="list">
      {movies?.map((movie) => (
...
function Movie({ movie }: { movie: MovieData }): JSX.Element {
  return (
    <li key={movie.imdbID}>
...
// Это пример HOC, который принимает компонент и добавляет к нему новые пропсы
/* function withExtraProps<P>(WrappedComponent: React.ComponentType<P>) {
  return function WithExtraProps(props: P) {
    ...
    const extraProps = {
      extraProp1: 'Значение1',
      extraProp2: 'Значение2',
    };

    // Оборачиваем WrappedComponent с дополнительными пропсами
